function [x xd t xT x_obs]=Simulation(x0,xT,fn_handle,v_exp,x_exp,varargin)
%
% This function simulates motion that were learnt using , which defines
...
if options.plot %plotting options
    sp = plot_results('i',[],x,x,obs);
end
%% Simulation
i=1; % 第几个点
while true
    err_sub(:,i,:)=reshape(fn_handle(squeeze(x_exp(:,i,:)) - squeeze(x(:,i,:))),[d 1 nbSPoint]);
    xd(:,i,:) = v_exp(:,i,:) - err_sub(:,i,:);
